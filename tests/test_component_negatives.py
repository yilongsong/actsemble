"""Negative generation: determinism, bounds, per-type behavior, guards."""
...
def test_all_enabled_types_produce_valid_negatives(episodes):
    ...
    for ntype in gen.cfg.enabled_types:
        ...
        neg, name = gen.generate(episodes, 0, 10, w.action_chunk, rng, negative_type=ntype)
        assert name == ntype
        assert neg.shape == w.action_chunk.shape
        assert np.isfinite(neg).all()
        assert (neg >= -1.0 - 1e-6).all() and (neg <= 1.0 + 1e-6).all()
        assert not np.allclose(neg, w.action_chunk), f"{ntype} produced the positive chunk"
...
def test_temporal_shift_is_a_real_chunk_elsewhere(episodes):
    ...
    candidates = [
        extract_window(episodes[2], 20 + d, obs_horizon=2, prediction_horizon=8).action_chunk
...
def test_gripper_mismatch_requires_gripper_dims():
    with pytest.raises(ValueError, match="gripper"):
        NegativeConfig.from_dict({"enabled_types": ["gripper_mismatch"], "gripper_dims": []})
...
def test_component_score_shape_and_dataset_hash(trained_checkpoints):
    ...
    chunks = torch.rand(5, int(comp.meta["prediction_horizon"]), int(comp.meta["action_dim"]))
...
def test_all_negative_types_registry_has_implementations(episodes):
    gen = _generator(episodes, enabled_types=list(ALL_NEGATIVE_TYPES[:-1]))  # sans gripper

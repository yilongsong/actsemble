"""System subsystem: candidate identity and selection/fallback behavior.
...
def test_selection_rules_differ_only_in_index(trained_checkpoints):
    ...
    actsemble = CandidateRerankingActsemble(
        policy, comp, num_candidates=6, candidate_root_seed=1
    )
...
def test_factory_builds_and_validates(trained_checkpoints):
    ...
    with pytest.raises(ValueError, match="num_candidates"):
        MultiSampleControlSystem(policy, num_candidates=0)


def test_candidate_hashes_identical_across_all_three_paired_systems(
    trained_checkpoints,
):
...
def test_candidate_seed_depends_on_checkpoint_hash(trained_checkpoints):
    ...
    best = DiffusionPolicy.from_checkpoint(
        trained_checkpoints["policy_best"], device="cpu"
    )
    final = DiffusionPolicy.from_checkpoint(
        trained_checkpoints["policy_final"], device="cpu"
    )

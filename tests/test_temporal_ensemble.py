"""Unit tests for A4 temporal ensembling (no policy training / GPU needed).
...
def test_recency_oldest_vs_recent_flips_weighting():
    preds = np.array([[0.0], [10.0]])  # row 0 = freshest (age 0), row 1 = oldest (age 1)
    ages = np.array([0, 1])
    recent = aggregate_predictions(preds, ages, decay=1.0, mode="mean", recency="recent")
    oldest = aggregate_predictions(preds, ages, decay=1.0, mode="mean", recency="oldest")
    ...
    system = build_system(
        {
            ...
            "selection": {"type": "temporal_ensemble", "aggregation": aggregation, "decay": 0.2},

"""Protocol subsystem: confirmation and lexicographic selection rules
...
class TestPolicyLexicographicSelection:
    ...
    def test_highest_confirmation_wins(self):
        winner = select_policy(
            [self._cand(2000, 0.9, 0.50), self._cand(4000, 0.5, 0.60)]
        )
        assert winner["step"] == 4000

    def test_confirmation_tie_broken_by_screening(self):
        winner = select_policy(
            [self._cand(2000, 0.55, 0.60), self._cand(4000, 0.65, 0.60)]
        )
        assert winner["step"] == 4000
...
class TestVerifierOfflineSelection:
    ...
    def _hist(step, ranking, balanced=0.9, loss=0.1):
        return {
            "step": step,
            "checkpoint_path": f"v/step_{step:06d}.pt",
    ...
    def test_best_primary_wins(self):
        w = select_verifier_record(
            [self._hist(100, 0.90), self._hist(200, 0.95), self._hist(300, 0.93)]
        )
        assert w["step"] == 200

    def test_secondary_breaks_primary_tie(self):
        w = select_verifier_record(
            [self._hist(100, 0.95, balanced=0.90), self._hist(200, 0.95, balanced=0.94)]
        )
        assert w["step"] == 200

    def test_complete_tie_prefers_earliest(self):
        w = select_verifier_record(
            [self._hist(300, 0.95), self._hist(100, 0.95), self._hist(200, 0.95)]
        )
        assert w["step"] == 100
    ...
    def test_validation_loss_primary_prefers_lower(self):
        w = select_verifier_record(
            ...
            secondary="pairwise_ranking_accuracy",
        )
        assert w["step"] == 200

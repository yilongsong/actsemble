"""Protocol subsystem: fixed-panel determinism, structure, and disjointness."""
...
def test_default_panels_match_protocol_recipe():
    panels = load_panels()
    assert (
        panels["screening"].env_seed == 2000 and panels["screening"].num_episodes == 50
    )
    assert (
        panels["confirmation"].env_seed == 3500
        and panels["confirmation"].num_episodes == 200
    )
    assert (
        panels["integration"].env_seed == 4500
        and panels["integration"].num_episodes == 10
    )
    assert (
        panels["final_test"].env_seed == 1000
        and panels["final_test"].num_episodes == 500
    )
...
def test_same_root_would_collide():
    with pytest.raises(ValueError, match="not disjoint"):
        assert_panels_disjoint({"a": Panel("a", 42, 10), "b": Panel("b", 42, 10)})

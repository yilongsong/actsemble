"""Protocol subsystem: mid-training evaluation must not affect training.
...
def _final_weights(summary):
    ckpt = torch.load(
        summary["checkpoints"]["final"], map_location="cpu", weights_only=False
    )
...
def test_screening_frequency_does_not_change_training(tmp_path):
    ...
    every2 = _train(
        tmp_path, "screen_every_2", checkpoint_every=2, on_checkpoint=_noisy_callback
    )
    every5 = _train(
        tmp_path, "screen_every_5", checkpoint_every=5, on_checkpoint=_noisy_callback
    )
    ...
    assert (
        ...
        == every2["final_train_loss"]
        == every5["final_train_loss"]
    )

"""Protocol subsystem: overwrite protection, freezing, and verifier audit
trail (§10, §16, §17)."""
...
def test_verify_result_against_freeze_catches_mismatches():
    ...
    for key, bad_value in [
        ("policy_checkpoint_hash", "X"),
        ("num_candidates", 16),
        ("dataset_hash", "X"),
        ("component_checkpoint_hashes", ["X"]),
    ]:
        problems = verify_result_against_freeze({**good, key: bad_value}, manifest)
        assert problems, f"{key} mismatch not caught"


def test_v2_freeze_checks_sampler_source_panel_and_component_presence():
    manifest = {
        "protocol": "actsemble_system_freeze_v2",
        "policy": {
            "checkpoint_hash": "P",
            "weights_kind": "ema",
            "sampler_provenance": {"sampler": "ddim"},
        },
        "verifier": {"checkpoint_hash": "V"},
        "dataset": {"dataset_hash": "D"},
        "environment": {"task_id": "T", "controller": "c", "simulation_backend": "b"},
        "system": {
            "num_candidates": 4,
            "allowed_selection_types": ["highest_component_score"],
        },
        "source": {"source_tree_hash": "S"},
        "panels": {
            "final_test": {"name": "final_test", "env_seed": 1, "num_episodes": 2}
        },
    }
    good = {
        ...
        "task": "T",
        "num_candidates": 4,
        "selection_type": "highest_component_score",
        "component_checkpoint_hashes": ["V"],
        "sampler": {"sampler": "ddim"},
        "source": {"source_tree_hash": "S"},
        "panel": {"name": "final_test", "env_seed": 1, "num_episodes": 2},
    }
    assert verify_result_against_freeze(good, manifest) == []
    for mutation in (
        {"result_schema": "actsemble_evaluation_v1"},
        {"component_checkpoint_hashes": []},
        {"sampler": {"sampler": "ddpm"}},
        {"source": {"source_tree_hash": "dirty"}},
        {"panel": {"name": "final_test", "env_seed": 2, "num_episodes": 2}},
    ):
        assert verify_result_against_freeze({**good, **mutation}, manifest)


def test_built_freeze_records_exact_sampler_and_source(trained_checkpoints):
    from actsemble.data.writer import write_private_provenance
    from actsemble.evaluation.panels import Panel
    from actsemble.protocol.freeze import build_freeze_manifest

    dataset = trained_checkpoints["dataset"]
    write_private_provenance(
        dataset,
        {
            "success_only": True,
            "exported_episodes": [
                {"source_episode_seed": 9000, "source_success": True}
            ],
        },
    )
    panels = {
        "integration": Panel("integration", 4500, 2),
        "final_test": Panel("final_test", 1000, 3),
    }
    manifest = build_freeze_manifest(
        policy_path=trained_checkpoints["policy_best"],
        verifier_path=trained_checkpoints["component_best"],
        dataset_path=dataset,
        panels=panels,
        num_candidates=4,
        device="cpu",
    )
    assert manifest["protocol"] == "actsemble_system_freeze_v2"
    assert manifest["policy"]["sampler_provenance"]["timesteps"]
    assert len(manifest["source"]["source_tree_hash"]) == 64
    assert manifest["policy"]["config_hash"]
...
def test_selected_verifier_carries_audit_trail(trained_checkpoints, tmp_path):
    ...
    history = [
        {
            "step": 10,
            "checkpoint_path": str(snap),
            "evaluated_on": "validation_episodes",
            "metrics": {
                "pairwise_ranking_accuracy": 0.9,
    ...
    ckpt = torch.load(
        run_dir / "selected_verifier.pt", map_location="cpu", weights_only=False
    )

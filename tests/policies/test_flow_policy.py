"""Policy subsystem: flow-matching tests (CPU, tiny U-Net, no dataset/GPU).
...
def test_sample_shape_clip_and_determinism():
    ...
    a = policy.sample_action_chunks(
        obs, num_samples=4, generator=policy.new_generator(0)
    )
    b = policy.sample_action_chunks(
        obs, num_samples=4, generator=policy.new_generator(0)
    )
...
# ---- checkpoint + loader ---------------------------------------------------
def test_from_checkpoint_sampler_overrides_change_inference_steps(tmp_path):
    meta, _ = _meta()
    model = _model()
    path = tmp_path / "flow.pt"
    FlowMatchingPolicy.save_checkpoint(
        path,
        config={"model": {"channels": [16, 32], "diffusion_embedding_dim": 16, "kernel_size": 3},
                "flow": {"inference_steps": 10}},
        meta=meta, model_state=model.state_dict(), ema_state=model.state_dict(),
    )
    assert FlowMatchingPolicy.from_checkpoint(path, device="cpu").num_inference_steps == 10
    ov = FlowMatchingPolicy.from_checkpoint(
        path, device="cpu", sampler_overrides={"inference_steps": 5}
    )
    assert ov.num_inference_steps == 5  # Euler step-count sweep uses this
...
def test_checkpoint_roundtrip_and_loader(tmp_path):
    ...
    assert torch.allclose(
        loaded.sample_action_chunks(
            obs, num_samples=1, generator=loaded.new_generator(0)
        ),
        ref.sample_action_chunks(obs, num_samples=1, generator=ref.new_generator(0)),
        atol=1e-6,
    )

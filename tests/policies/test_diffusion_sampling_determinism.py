"""Policy subsystem: diffusion determinism, EMA, clipping, and extraction."""
...
def _obs(policy, seed=0):
    ...
    return rng.uniform(
        -1, 1, size=(policy.meta.obs_horizon, policy.meta.state_dim)
    ).astype(np.float32)
...
def test_same_seed_same_samples(trained_checkpoints):
    ...
    a = policy.sample_action_chunks(
        obs, num_samples=4, generator=policy.new_generator(3)
    )
...
def test_different_seeds_differ(trained_checkpoints):
    ...
    b = policy.sample_action_chunks(
        obs, num_samples=4, generator=policy.new_generator(4)
    )
...
def test_candidates_within_one_call_are_independent(trained_checkpoints):
    ...
    assert not any(
        torch.equal(flat[i], flat[j]) for i in range(8) for j in range(i + 1, 8)
    )
...
def test_ddpm_sampler_also_deterministic(trained_checkpoints):
    policy = _policy(
        trained_checkpoints,
        sampler_overrides={"inference_sampler": "ddpm", "inference_steps": 5},
    )
    obs = _obs(policy)
    a = policy.sample_action_chunks(
        obs, num_samples=2, generator=policy.new_generator(9)
    )
    b = policy.sample_action_chunks(
        obs, num_samples=2, generator=policy.new_generator(9)
    )

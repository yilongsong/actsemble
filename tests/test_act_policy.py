"""Unit tests for the ACT policy (CPU, tiny model, no dataset/GPU).
...
def _model(**kw):
    return ACTModel(
        obs_feature_dim=SD, action_dim=AD, obs_horizon=HO, prediction_horizon=HP,
        hidden_dim=kw.get("hidden_dim", 32), latent_dim=kw.get("latent_dim", 8),
        n_heads=2, n_encoder_layers=1, n_decoder_layers=1, dim_feedforward=32,
...
def _policy(model=None):
    meta, norm = _meta()
    return ACTPolicy(model or _model(), norm, meta, config={"model": {}}, device=torch.device("cpu"))
...
def test_kl_zero_at_standard_normal():
    ...
    assert torch.allclose(ACTModel.kl_divergence(mu, logvar), torch.tensor(0.0), atol=1e-6)
...
def test_sample_rejects_wrong_obs_shape():
    ...
    with pytest.raises(ValueError, match="observation_history shape"):
        policy.sample_action_chunks(np.zeros((HO, SD + 1), np.float32), num_samples=1, generator=None)
...
def test_checkpoint_roundtrip(tmp_path):
    ...
    ACTPolicy.save_checkpoint(
        path, config={"model": {"hidden_dim": 32, "latent_dim": 8, "n_heads": 2,
    ...
    ref = ACTPolicy(model, Normalizer(NormalizationStats.from_dict(meta.normalization)), meta,
                    config={}, device=torch.device("cpu"))
    assert torch.allclose(
        loaded.sample_action_chunks(obs_hist, num_samples=1, generator=None),
        ref.sample_action_chunks(obs_hist, num_samples=1, generator=None), atol=1e-6,
    )
...
def test_build_act_model_reads_config():
    meta, _ = _meta()
    m = build_act_model({"model": {"hidden_dim": 64, "latent_dim": 16, "n_encoder_layers": 2}}, meta)
...
def test_temporal_ensemble_runs_over_act():
    ...
    for i in range(6):
        obs = StateObservation(state=rng.uniform(-1, 1, SD).astype(np.float32),
                               previous_action=np.zeros(AD, np.float32), step_index=i)

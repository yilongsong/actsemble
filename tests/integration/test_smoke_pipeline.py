"""Integration subsystem: dataset -> train both -> assemble -> act.
...
def test_full_pipeline_without_sim(trained_checkpoints):
    ...
    assert (
        component.dataset_hash
        == trained_checkpoints["component_summary"]["dataset_hash"]
    )
...
def test_component_scores_prefer_real_chunks_after_training(trained_checkpoints):
    ...
    corrupted = torch.from_numpy(
        np.clip(
            w.action_chunk + rng.uniform(-1, 1, w.action_chunk.shape), -1, 1
        ).astype(np.float32)
    ).unsqueeze(0)

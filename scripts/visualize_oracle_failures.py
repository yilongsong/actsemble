...
base = {e["env_seed"]: e for e in json.load(open(OUT / "base_0000_0300.json"))["episodes"]}
orac = {}
...
CCOL = {"never engaged": "#8d99ae", "near miss (fell back)": "#e76f51",
        "misaligned at goal": "#e9c46a", "mispositioned": "#457b9d"}
...
def bar(ax, labels_counts, title):
    ...
    ax.set_title(title); ax.set_ylabel("# episodes"); ax.grid(alpha=0.25, axis="y", lw=0.5)
...
for e in base_fail.values():
    c = categorize(e); cats1[c] = cats1.get(c, 0) + 1
goal = np.asarray(next(iter(base.values()))["goal_xy"])

fig, ax = plt.subplots(1, 3, figsize=(16.5, 5.2))
fig.suptitle(f"Failure modes WITHOUT oracle — base candidate_zero policy "
             f"({len(base_fail)}/{len(base)} episodes fail)", fontsize=13, fontweight="bold")
bar(ax[0], cats1, "(a) Failure-mode counts")

for es, e in base_fail.items():
    c = categorize(e); obj = np.asarray(e["obj_xy"])
...
handles = [plt.Line2D([], [], marker="o", ls="", color=CCOL[c], label=c) for c in CATS if c in cats1]
ax[1].legend(handles=handles, fontsize=7, loc="upper right")

for es, e in base_fail.items():
    c = categorize(e); cov = np.asarray(e["coverage"])
...
ax[2].set_xlabel("normalized time"); ax[2].set_ylabel("goal-area coverage")
ax[2].set_ylim(0, 1); ax[2].legend(fontsize=8); ax[2].grid(alpha=0.25, lw=0.5)
fig.tight_layout(rect=[0, 0, 1, 0.94]); fig.savefig(FIG / "fail1_base_modes.png", dpi=125); plt.close(fig)

# ================== FIG 2: base fails, oracle recovers (headroom made concrete) ==================
recov = [es for es in orac if es in base and not base[es]["success_once"] and orac[es]["success_once"]]
recov = sorted(recov, key=lambda es: -np.asarray(base[es]["coverage"]).max())  # base got closest first
pick = recov[:8]
fig, ax = plt.subplots(2, 4, figsize=(16.5, 8.4))
fig.suptitle(f"Base policy FAILS → Oracle RECOVERS (same init): selection headroom made concrete "
             f"({len(recov)} such episodes in the {len(orac)} shown)", fontsize=13, fontweight="bold")
for a, es in zip(ax.flat, pick):
    ...
    bc = np.asarray(base[es]["coverage"]).max(); oc = np.asarray(orac[es]["coverage"]).max()
    a.set_title(f"seed {es}\nbase max cov {bc:.2f} · oracle {oc:.2f}", fontsize=8)
    a.set_aspect("equal"); a.grid(alpha=0.25, lw=0.5); a.tick_params(labelsize=6)
...
fig.tight_layout(rect=[0, 0, 1, 0.93]); fig.savefig(FIG / "fail2_oracle_recovers.png", dpi=125); plt.close(fig)

# ============================ FIG 3: oracle failure modes ============================
orac_fail = {es: e for es, e in orac.items() if not e["success_once"]}
cats3 = {}
for e in orac_fail.values():
    c = categorize(e); cats3[c] = cats3.get(c, 0) + 1

fig, ax = plt.subplots(1, 3, figsize=(16.5, 5.2))
fig.suptitle(f"Failure modes OF the oracle — proposal-limited "
             f"({len(orac_fail)}/{len(orac)} oracle episodes still fail)", fontsize=13, fontweight="bold")
bar(ax[0], cats3, "(a) Oracle failure-mode counts")

for es, e in orac_fail.items():
    c = categorize(e); draw_T_paths(ax[1], e, CCOL[c], lw=0.8, mark=False)
    obj = np.asarray(e["obj_xy"]); ax[1].plot(*obj[-1], "o", ms=4, color=CCOL[c], alpha=0.7)
ax[1].plot(*goal, "*", ms=22, color="#e9c46a", mec="#7a5901", mew=1.2, zorder=10)
ax[1].set_title("(b) Oracle-failure T-block paths"); ax[1].set_aspect("equal")
ax[1].set_xlabel("x (m)"); ax[1].set_ylabel("y (m)"); ax[1].grid(alpha=0.25, lw=0.5)
...
ax[2].hist(wf_fail, bins=np.linspace(0, 1, 21), color="#c1121f", alpha=0.7,
           label=f"oracle FAIL (n={len(wf_fail)})")
ax[2].hist(wf_succ, bins=np.linspace(0, 1, 21), color="#2a9d8f", alpha=0.6,
           label=f"oracle SUCCESS (n={len(wf_succ)})")
ax[2].set_title("(c) Fraction of replans with ≥1 winning candidate\n(≈0 for failures ⇒ proposal-limited)")
ax[2].set_xlabel("fraction of replans where some candidate reaches goal")
ax[2].set_ylabel("# episodes"); ax[2].legend(fontsize=8); ax[2].grid(alpha=0.25, lw=0.5)
fig.tight_layout(rect=[0, 0, 1, 0.94]); fig.savefig(FIG / "fail3_oracle_modes.png", dpi=125); plt.close(fig)

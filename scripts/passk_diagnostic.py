...
spec = importlib.util.spec_from_file_location("oh", str(REPO / "scripts/oracle_headroom.py"))
...
report = {"n_episodes": len(rows), "kmax": KMAX,
          "pass_at_1": curve[1], "pass_at_16": curve[16],
...
print(f"  episodes never solved in 16 tries: {report['episodes_with_zero_success']}/{len(rows)}")
print(f"  episodes solved by ALL 16 samples: {report['episodes_with_all16_success']}/{len(rows)}")

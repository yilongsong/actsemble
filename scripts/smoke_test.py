...
def main() -> int:
    ...
    parser.add_argument(
        "--workdir", default=None, help="working directory (default: temp)"
    )
    parser.add_argument("--keep", action="store_true", help="keep workdir on success")
    parser.add_argument(
        "--episodes", type=int, default=8, help="source demos to convert"
    )
    ...
    workdir = (
        Path(args.workdir)
        if args.workdir
        else Path(tempfile.mkdtemp(prefix="actsemble_smoke_"))
    )
    ...
    component_cfg = load_config(
        repo / "configs" / "components" / "action_chunk_compatibility.yaml"
    )
    ...
    from actsemble.sim.env_factory import (
    ...
    metadata = DatasetMetadata(
        ...
        state_layout=json.dumps(layout),
        controller=contract["controller"],
        action_dimension=episodes[0].action.shape[1],
        action_definition=json.dumps(
            {
                "semantics": "normalized EE delta pose",
                "frame": "root aligned",
                "units": "[-1,1]",
                "bounds": [
    ...
    ds = DiffusionWindowDataset(
        reader.episodes,
        Normalizer(compute_stats(reader.episodes)),
        obs_horizon=2,
    ...
    policy_out = train_diffusion_policy(
        ...
        output_dir=workdir / "policy",
        max_steps=args.train_steps,
        device=device,
    )
    check(
        "policy training",
        policy_out["steps"] == args.train_steps,
    ...
    chunks = policy.sample_action_chunks(
        obs_hist, num_samples=4, generator=policy.new_generator(0)
    )
    ...
    check(
        ...
        and (chunks >= low - 1e-6).all()
        and (chunks <= high + 1e-6).all(),
    )

    chunks2 = policy.sample_action_chunks(
        obs_hist, num_samples=4, generator=policy.new_generator(0)
    )
    ...
    comp_out = train_component(
        ...
        dataset_path=dataset_path,
        output_dir=workdir / "component",
        max_steps=args.train_steps,
        device=device,
    )
    check(
        "component training",
        comp_out["steps"] == args.train_steps,
    ...
    system_cfgs = {
        "standalone": load_config(
            repo / "configs" / "systems" / "standalone_diffusion.yaml"
        ),
        "control": load_config(
            repo / "configs" / "systems" / "multisample_control.yaml"
        ),
        "actsemble": load_config(
            repo / "configs" / "systems" / "compatibility_reranking.yaml"
        ),
    }
    systems = {
    ...
    check(
        "results share dataset hash",
        all(r["dataset_hash"] == dataset_hash for r in results.values()),
    )
    check(
        "results share policy checkpoint hash",
        len({r["policy_checkpoint_hash"] for r in results.values()}) == 1,
    )
    check(
        "paired seeds identical across systems",
        all(
            r["environment_seeds"] == results["standalone"]["environment_seeds"]
    ...
    check(
        ...
        "pairwise_vs_baseline" in report,

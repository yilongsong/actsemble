...
def exec_offset(meta) -> int:
    return meta.obs_horizon - 1 if meta.extra.get("window_alignment") == "diffusion_policy" else 0


def main() -> int:
    ap = argparse.ArgumentParser(description=__doc__)
    ap.add_argument("--count", type=int, default=300)
    ap.add_argument("--out-dir", default=str(REPO / "outputs/active_min/canonical_eval"))
    ...
    env = make_env(task_id=ref.meta.task_id, control_mode=ref.meta.controller,
                   sim_backend=ref.meta.simulation_backend, obs_mode="state", render_mode="rgb_array")
    del ref
    ...
    for label, ckpt in POLICIES.items():
        ...
        cfg = {"name": f"standalone_{label}", "policy": {"num_candidates": 1}, "components": [],
               "selection": {"type": "candidate_zero"}, "execution": {"action_offset": offset}}
        print(f"[eval-canonical] {label}  (offset {offset}, norm {meta.normalization['method']}, "
              f"obs_horizon {meta.obs_horizon}) ...", flush=True)
        r = evaluate_system(system_cfg=cfg, eval_cfg=eval_cfg, policy_checkpoint=str(ckpt),
                            num_episodes=args.count, output_path=out / f"eval_{label}.json",
                            device=args.device, env=env, force=True)
        ci = wilson_interval(r["success_count"], args.count)
        rows[label] = {"success_rate": r["success_rate"], "wilson_ci": list(ci),
                       "policy_ms": 1000 * r["latency"]["mean_policy_s"],
                       "offset": offset, "norm": meta.normalization["method"]}
    env.close()

    L = ["=" * 70, f"Authoritative state policies — diagnostic panel, n={args.count}", "=" * 70,
         f"{'policy':<12}{'success':>10}{'wilson 95% CI':>20}{'infer ms/query':>16}"]
    L.append("-" * 58)
    for label, s in rows.items():
        ci = s["wilson_ci"]
        L.append(f"{label:<12}{s['success_rate']:>9.1%}{f'[{ci[0]:.1%},{ci[1]:.1%}]':>20}"
                 f"{s['policy_ms']:>15.1f}")
    print("\n".join(L))

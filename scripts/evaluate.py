...
def main() -> int:
    ...
    parser.add_argument("--component-checkpoint", action="append", default=[],
    ...
    print(f"[evaluate] success ({result['primary_metric']}): "
    ...
    print(f"[evaluate] timeouts: {result['timeout_rate']:.1%}  "
          f"exceptions: {result['exception_rate']:.1%}  "
          f"fallback episodes: {result['fallback_rate']:.1%}")
    print(f"[evaluate] mean decision latency: "

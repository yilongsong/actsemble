...
class OracleRollout:
    ...
    def _step(self, e, action_vec):
        env_action = self.act.adapt(
            RobotAction(value=np.asarray(action_vec, dtype=np.float32))
        )
    ...
    def _set_benv(self, state):
        ...
        if torch.is_tensor(esc):
            self.bu._elapsed_steps = torch.zeros_like(
                esc
            )  # backing field (property has no setter)
    ...
    def base_episode(self, env_seed, policy_sampling_seed, render=False):
        ...
        while step < MAX_STEPS and not success_once:
            ...
            for a in chunk[off0 : off0 + self.h_a]:
                ...
                tcp_xy.append(s[14:16].copy())
                cov.append(self._cov(self.env))
                if render:
    ...
    def _capture_row(
        self, env_seed, pss, success, step, obj_xy, tcp_xy, cov, last_state
    ):
        return {
            "env_seed": int(env_seed),
            "policy_sampling_seed": int(pss),
            "success_once": bool(success),
            "num_steps": int(step),
            ...
            "final_obj_quat": np.asarray(last_state[27:31], np.float32)
            .round(5)
            .tolist(),
        }
    ...
    def episode(self, env_seed, policy_sampling_seed, capture=False, render=False):
        ...
        self.benv.reset(
            seed=int(env_seed)
        )  # scratch env (config; overwritten by set_state)
        s0 = self._state(raw)
        hist = [s0]
        obj_xy, tcp_xy, cov = (
            [s0[24:26].copy()],
        ...
        row = {
            "env_seed": int(env_seed),
            "policy_sampling_seed": int(policy_sampling_seed),
...
def cmd_run(args):
    ...
    for i, ep in enumerate(sel):
        ...
        if (i + 1) % 5 == 0 or i == 0:
            ...
            print(
                f"[oracle K{k}M{m}] {start + i + 1}/{start + len(sel)} run={rate:.1%} "
    ...
    print(
        f"[oracle K{k}M{m}] wrote {dest.name}: {np.mean([r['success_once'] for r in rows]):.1%}"
    )
    return 0
...
def cmd_capture(args):
    ...
    if dest.exists() and not args.force:
        print(f"[capture] {dest.name} exists; skipping")
        return 0
    ...
    for i, ep in enumerate(sel):
        ...
        if (i + 1) % 20 == 0:
            print(
                f"[capture:{mode}] {start + i + 1}/{start + len(sel)} ({(time.time() - t0) / (i + 1):.1f}s/ep)",
                flush=True,
            )
    env.close()
    benv.close()
    ...
    print(
        f"[capture:{mode}] wrote {dest.name} ({len(rows)} eps, {np.mean([r['success_once'] for r in rows]):.1%})"
    )
    return 0
...
def _filmstrip(path, es, label, fb, rb, fo, ro):
    ...
    fig.suptitle(
        f"seed {es}  ({label})   base={'SUCCESS' if rb['success_once'] else 'FAIL'} "
        f"(maxcov {np.asarray(rb['coverage']).max():.2f})   →   "
        f"oracle={'SUCCESS' if ro['success_once'] else 'FAIL'} "
        f"(maxcov {np.asarray(ro['coverage']).max():.2f})",
        fontsize=11,
        fontweight="bold",
    )
...
def cmd_video(args):
    ...
    if args.seeds:
        ...
        cap = OUT / "capture"
        base = {
            e["env_seed"]: e for e in load_json(cap / "base_0000_0300.json")["episodes"]
        }
        orac = {}
        ...
        rn = pick(
            lambda es: _categorize(base[es]) == "near_miss" and orac[es]["success_once"]
        )
        rm = pick(
            lambda es: (
                _categorize(base[es]) == "mispositioned" and orac[es]["success_once"]
            )
        )
        of = pick(
            lambda es: (
                _categorize(orac[es]) == "mispositioned"
                and not orac[es]["success_once"]
            )
        )
        labels = {
            rn: "recovered_near_miss",
            rm: "recovered_mispositioned",
            of: "oracle_fail_proposal_limited",
        }
        seeds = [s for s in (rn, rm, of) if s is not None]

    for es in seeds:
        rb = roll.base_episode(es, pss[es], render=True)
        fb = list(roll.frames)
        save_video(fb, vdir / f"seed{es}_{labels[es]}_base.mp4")
        ro = roll.episode(es, pss[es], capture=True, render=True)
        fo = list(roll.frames)
        save_video(fo, vdir / f"seed{es}_{labels[es]}_oracle.mp4")
        _filmstrip(vdir / f"seed{es}_{labels[es]}.png", es, labels[es], fb, rb, fo, ro)
        print(
            f"[video] seed {es} {labels[es]}: base={'S' if rb['success_once'] else 'F'} "
            f"oracle={'S' if ro['success_once'] else 'F'}  ({len(fb)}|{len(fo)} frames)",
            flush=True,
        )
    env.close()
    benv.close()
...
def _load_oracle(k, m):
    ...
    if tag:
        ...
        shards = [
            ...
            if not p.name.startswith("shard_K")
        ]
    rows = {}
    for s in shards:
...
def cmd_analyze(args):
    ...
    base_rows = (
        _load_oracle(K, 1) if _tag(k, m) else rows
    )  # baseline oracle for the shift

    cz = load_json(CZ_JSON)
    ve = load_json(VE_JSON)
    eps = panel_episodes(PANEL)
    ...
    oracle_outcomes = np.array([oracle[es] for es in paired], int)
    C = np.array([cz_by[es] for es in paired], int)
    V = np.array([ve_by[es] for es in paired], int)
    B = np.array(
        [base_rows[es]["success_once"] for es in paired if es in base_rows], int
    )
    n = len(paired)
    rO, rC, rV = oracle_outcomes.mean(), C.mean(), V.mean()
    ...
    for es in paired:
        for rp in rows[es]["replans"]:
            ...
            tot_contact += contact
            tot_free += not contact
            if rp["changed_from_zero"]:
                ch_contact += contact
                ch_free += not contact

    contrasts = {
        "oracle_vs_cz": _mcnemar(oracle_outcomes, C),
        "oracle_vs_verifier": _mcnemar(oracle_outcomes, V),
        "verifier_vs_cz": _mcnemar(V, C),
    }
    if _tag(k, m) and len(B) == n:  # variant vs baseline oracle: the ceiling shift
        contrasts["oracle_vs_baseline_oracle"] = _mcnemar(oracle_outcomes, B)
    report = {
        ...
        "contrasts": contrasts,
        "selection_change": {
            "changed_at_contact": ch_contact,
            "changed_at_freespace": ch_free,
            "total_contact_replans": tot_contact,
            "total_freespace_replans": tot_free,
        },
    }
    save_json(
        report, OUT / f"headroom_{_tag(k, m) or 'K16_M1_'}.json".replace("__", "_")
    )
    ...
    if _tag(k, m) and rB == rB:
        print(
            f"  baseline Oracle@16 (M=1) on same eps : {rB:.1%}   ceiling shift {rO - rB:+.1%}"
        )
    ...
    for name, c in contrasts.items():
        print(
            f"  {name:26s} z={c['z']:+.2f} ({'sig' if c['sig'] else 'ns'}; +{c['a_wins']}/-{c['b_wins']})"
        )
    sc = report["selection_change"]
    print(
        f"  oracle changed pick: contact {sc['changed_at_contact']}/{sc['total_contact_replans']}, "
...
def main():
    ...
    p.add_argument(
        "--m", default=1, help="Monte-Carlo continuations per candidate (variance axis)"
    )

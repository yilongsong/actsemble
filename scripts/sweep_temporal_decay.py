...
def run_sweep(policy_ckpt, decays, count, panel_name, out_dir: Path, device):
    ...
    for m in decays:
        ...
        print(f"[sweep] temporal_mean decay={m} on {panel_name} ({count} ep) ...", flush=True)
        evaluate_system(
            system_cfg=cfg, eval_cfg=eval_cfg, policy_checkpoint=policy_ckpt,
            num_episodes=count, output_path=out_dir / f"eval_temporal_mean_m{m:.2f}.json",
...
def build_and_report(out_dir: Path) -> dict | None:
    ...
    means = [load_json(p) for p in sorted(glob.glob(str(out_dir / "eval_temporal_mean*.json")))]
    ...
    sweep = [{**row(r, f"mean@m={_decay_of(r):g}"), "decay": _decay_of(r)} for r in means]
    report = {
        "panel": means[0]["panel"]["name"], "num_episodes": n,
...
def _format(rep: dict) -> str:
    L = ["=" * 72, f"ACT temporal-ensemble decay sweep — {rep['panel']} panel, n={rep['num_episodes']}",
         "=" * 72]
    if rep["baseline"]:
        b = rep["baseline"]; ci = b["wilson_ci"]
        L.append(f"baseline (standalone) {b['success_rate']:>7.1%}  [{ci[0]:.1%},{ci[1]:.1%}]")
    if rep["latest"]:
        lt = rep["latest"]; ci = lt["wilson_ci"]
        L.append(f"temporal_latest (m=inf){lt['success_rate']:>6.1%}  [{ci[0]:.1%},{ci[1]:.1%}]"
                 + (f"  vs base {lt['diff_vs_baseline']:+.1%} p={lt['mcnemar_vs_baseline']['p_exact']:.3f}"
                    if "diff_vs_baseline" in lt else ""))
    L.append("-" * 72)
    ...
    for s in rep["sweep"]:
        ...
        L.append(f"{s['decay']:>9g}{s['success_rate']:>9.1%}{f'[{ci[0]:.1%},{ci[1]:.1%}]':>20}{extra:>16}")
    return "\n".join(L)
...
def _plot(rep: dict, path: Path) -> None:
    ...
    ax.errorbar(ms, ys, yerr=[lo, hi], marker="o", color="#2a6f97", capsize=3,
                lw=2, label="temporal_mean (ACT) @ m")
    if rep["baseline"]:
        b = rep["baseline"]
        ax.axhspan(b["wilson_ci"][0], b["wilson_ci"][1], color="#999999", alpha=0.18)
        ax.axhline(b["success_rate"], color="#555555", ls="-", lw=1.2,
                   label=f"baseline (H_a=8) {b['success_rate']:.0%}")
    if rep["latest"]:
        ax.axhline(rep["latest"]["success_rate"], color="#c1121f", ls="--", lw=1.4,
                   label=f"temporal_latest (m=inf) {rep['latest']['success_rate']:.0%}")
    ax.set_xlabel("decay m   (0 = uniform average  →  large = only freshest)")
    ax.set_ylabel("success rate")
    ax.set_title(f"ACT temporal-ensemble decay sweep — {rep['panel']} panel (n={rep['num_episodes']})")
...
def main() -> int:
    ...
    if not args.report_only:
        ...
        run_sweep(args.policy_checkpoint, decays, args.count, args.panel, out_dir, args.device)

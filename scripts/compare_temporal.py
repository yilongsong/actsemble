...
Two comparisons, with the right fairness control for each (docs/system_architecture.md §2.5):
  ...
  2. AGGREGATION KNOB — the ensembling variants (mean/projection/medoid) vs
     ...
     same-plan property is the OPEN-loop one proven in tests/test_temporal_ensemble.py.
...
CONTROL = "temporal_latest"  # replan-every-step, no ensembling (compute-matched control)
...
def run_all(policy_ckpt: str, count: int, decay: float, out_dir: Path, device: str,
            panel_name: str, labels: list[str]) -> dict:
    ...
    for label in labels:
        ...
        print(f"[compare_temporal] evaluating {label} on {panel_name} ({count} ep) ...", flush=True)
        results[label] = evaluate_system(
            system_cfg=cfg, eval_cfg=eval_cfg, policy_checkpoint=policy_ckpt,
            num_episodes=count, output_path=out_dir / f"eval_{label}.json",
...
def build_report(results: dict[str, dict], count: int, panel_name: str) -> dict:
    ...
    for label, r in results.items():
        ...
        rows[label] = {
            "success_rate": r["success_rate"], "success_count": r["success_count"],
            "wilson_ci": list(ci),
    ...
    def paired(a_label: str, b_label: str) -> dict:
        ...
        return {
            ...
            "policy_calls_ratio": rows[a_label]["mean_replans"] / max(1e-9, rows[b_label]["mean_replans"]),
        }

    # PRIMARY: each temporal variant present vs the receding-horizon baseline (budget-matched)
    vs_baseline = ({label: paired(label, BASELINE) for label in TEMPORAL if label in results}
                   if BASELINE in results else {})
    # AGGREGATION KNOB: ensembling variants vs replan-every-step-only (compute-matched)
    vs_control = ({label: paired(label, CONTROL) for label in ENSEMBLE if label in results}
                  if CONTROL in results else {})
    return {
        "panel": panel_name, "num_episodes": count, "baseline": BASELINE, "control": CONTROL,
        "systems": rows, "temporal_vs_baseline": vs_baseline, "ensemble_vs_control": vs_control,
    }
...
def format_report(rep: dict) -> str:
    ...
    for label, s in rep["systems"].items():
        ci = s["wilson_ci"]
        L.append(f"{label:<24}{s['success_rate']:>8.1%}{f'[{ci[0]:.1%},{ci[1]:.1%}]':>20}"
    ...
    def block(pairs: dict) -> None:
        for label, p in pairs.items():
            ...
            L.append(f"  {label:<22} diff {p['abs_diff']:+.1%}{sig}  "
    ...
    if rep["temporal_vs_baseline"]:
        L.append("")
        L.append(f"PRIMARY — temporal vs {rep['baseline']} (budget-matched, env-paired; * = McNemar p<0.05):")
        block(rep["temporal_vs_baseline"])
    if rep["ensemble_vs_control"]:
        L.append("")
        L.append(f"AGGREGATION KNOB — ensembling vs {rep['control']} (compute-matched, env-paired):")
...
def main() -> int:
    ...
    results = run_all(args.policy_checkpoint, args.count, args.decay, Path(args.out_dir),
                      args.device, args.panel, labels)

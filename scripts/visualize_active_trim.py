...
def st(e):
    return np.asarray(full[e]["state"])


def ac(e):
    return np.asarray(full[e]["action"])
...
fig.suptitle(
    "Trailing-hold trim — full demos → push-only "
...
ax[0].hist(
    ...
    color=C_FULL,
    alpha=0.85,
    label=f"full (mean {orig_len.mean():.0f})",
)
ax[0].hist(
    kept_len,
    bins=bins,
    color=C_KEEP,
    alpha=0.85,
    label=f"push-only (mean {kept_len.mean():.0f})",
)
ax[0].set_title("(a) Episode length distribution")
ax[0].set_xlabel("timesteps")
ax[0].set_ylabel("# episodes")
ax[0].legend()

ax[1].scatter(
    first_s[~recov], kept_len[~recov], s=10, c=C_KEEP, alpha=0.5, label="normal"
)
ax[1].scatter(
    first_s[recov],
    kept_len[recov],
...
ax[1].set_xlabel("first success (90% coverage) step")
ax[1].set_ylabel("kept length")
...
ax[2].set_title(
    f"(c) Total transitions\nremoved {nb - na:,} ({(nb - na) / nb * 100:.1f}%)  ·  kept<18 steps: {stats['kept_below_18']}"
)
ax[2].set_ylabel("transitions")
ax[2].set_ylim(0, nb * 1.12)
for a in ax:
    a.grid(alpha=0.25, lw=0.5, axis="y")
fig.tight_layout(rect=[0, 0, 1, 0.94])
fig.savefig(f"{OUT}/trim_summary.png", dpi=125)
plt.close(fig)

# ================= FIG B: what got removed =================
fig, ax = plt.subplots(1, 3, figsize=(16.5, 5.0))
fig.suptitle(
    "What the trim removes — the low-energy hold tail; the push (and recovery) is kept",
    fontsize=13,
    fontweight="bold",
)
...
examples = (
    [(typical, "typical", "#264653"), (rec_eps[0], "recovery", C_REC)]
    if rec_eps
    else [(typical, "typical", "#264653")]
)
...
ax[0].set_title(
    "(a) T-block center-distance to goal\n● first true success   ┊ cut (kept end)"
)
ax[0].set_xlabel("timestep")
ax[0].set_ylabel("‖T−goal‖ (m)")
...
e_drop = np.array(
    [
        np.abs(ac(e)[info[e]["kept_length"] :]).mean()
        if info[e]["kept_length"] < info[e]["orig_length"]
        else np.nan
        for e in eps
    ]
)
...
ax[1].hist(
    e_full, bins=bins, color=C_FULL, alpha=0.8, label=f"full (μ={e_full.mean():.2f})"
)
ax[1].hist(
    e_keep,
    bins=bins,
    color=C_KEEP,
    alpha=0.75,
    label=f"kept/push (μ={e_keep.mean():.2f})",
)
ax[1].hist(
    e_drop[~np.isnan(e_drop)],
    bins=bins,
    color=C_CUT,
    alpha=0.6,
...
ax[1].set_title(
    "(b) Mean |action| per episode\nhold tail is low-energy; kept push is higher-energy"
)
ax[1].set_xlabel("mean |normalized action|")
...
ax[2].plot(
    *goal, "*", ms=20, color="#e9c46a", mec="#7a5901", mew=1.2, zorder=10, label="goal"
)
ax[2].plot([], [], color=C_FULL, lw=1.5, label="removed (hold)")
ax[2].plot([], [], color=C_KEEP, lw=1.5, label="kept (push)")
ax[2].set_aspect("equal")
ax[2].set_title("(c) T-block path: kept push vs removed hold")
ax[2].set_xlabel("x (m)")
ax[2].set_ylabel("y (m)")
...
fig.tight_layout(rect=[0, 0, 1, 0.94])
fig.savefig(f"{OUT}/trim_removed.png", dpi=125)
plt.close(fig)

...
def episode_from_traj(env_seed, success, obj_xy, coverage, goal_xy, num_steps, video=None):
    obj = np.asarray(obj_xy, float)
    cov = np.asarray(coverage, float)
    ftype = classify(success, obj_xy, coverage, goal_xy)
    return {
        "env_seed": int(env_seed), "success": bool(success), "failure_type": ftype,
        "final_cov": round(float(cov[-1]), 3), "max_cov": round(float(cov.max()), 3),
        "steps": int(num_steps),
        "pos_error": round(float(np.linalg.norm(obj[-1] - np.asarray(goal_xy, float))), 4),
        "obj_disp": round(float(np.linalg.norm(obj - obj[0], axis=1).max()), 4),
        "traj": {"obj": [[round(x, 4), round(y, 4)] for x, y in _ds(obj.tolist())],
...
def load_rich_capture(path, video_for=None):
    ...
    for r in rows:
        ...
        eps.append(episode_from_traj(r["env_seed"], r["success_once"], r["obj_xy"],
                                     r["coverage"], r["goal_xy"], r.get("num_steps", len(r["coverage"])), vid))
    return eps
...
def system_block(name, label, kind, color, episodes=None, success_map=None, goal_xy=None):
    """Build one system entry (rich episodes OR success-only)."""
    if episodes is None and success_map is not None:
        episodes = [{"env_seed": int(s), "success": bool(ok), "failure_type": "unknown",
                     "final_cov": None, "max_cov": None, "steps": None, "traj": None, "video": None}
    ...
    return {
        ...
        "metrics": {"success_rate": sc / n if n else None, "success_count": sc, "n": n,
...
def mcnemar(succ_map_a, succ_map_b):
    ...
    a1 = int(((a == 1) & (b == 0)).sum()); b1 = int(((a == 0) & (b == 1)).sum())
    disc = a1 + b1
    z = (a1 - b1) / np.sqrt(disc) if disc else 0.0
    return {"delta": float(a.mean() - b.mean()) if len(seeds) else 0.0,
            "mcnemar_z": float(z), "sig": bool(abs(z) > 1.96),
            "a_wins": a1, "b_wins": b1, "n": len(seeds)}
...
def cmd_build(args):
    ...
    if base_path.exists():
        eps = load_rich_capture(base_path, video_for=lambda es: relvid(base_vids.get(es)))
        systems.append(system_block("candidate_zero", "Candidate Zero (base)", "base", "#6c757d", episodes=eps))
    ...
    if orac_shards:
        ...
        for s in orac_shards:
            for r in load_json(s)["episodes"]:
                rows[r["env_seed"]] = r
        eps = [episode_from_traj(r["env_seed"], r["success_once"], r["obj_xy"], r["coverage"],
                                 r["goal_xy"], r.get("num_steps", len(r["coverage"])),
                                 relvid(orac_vids.get(r["env_seed"]))) for r in rows.values()]
        systems.append(system_block("oracle", "Oracle@16 (ceiling)", "oracle", "#2a9d8f", episodes=eps))
    # verifier / medoid — success-only from paired eval (rich via `record` later)
    for nm, label, color, fn in [
        ("verifier_argmax", "Verifier (learned)", "#6a4c93", cmp / "verifier_argmax.json"),
        ("full_chunk_medoid", "Medoid (consensus)", "#457b9d", cmp / "full_chunk_medoid.json"),
    ]:
        if fn.exists():
            systems.append(system_block(nm, label, "selector", color, success_map=load_success_only(fn)))

    # recorded rich captures (from `record`) override/extend if present
    for rc in sorted((DASH / "captures").glob("*.json")) if (DASH / "captures").exists() else []:
        ...
        blk = system_block(nm, meta.get("label", nm), meta.get("kind", "selector"),
                           meta.get("color", "#888"), episodes=eps)
    ...
    def add(a, b, note=""):
        if a in smaps and b in smaps:
            c = mcnemar(smaps[a], smaps[b]); c.update({"a": a, "b": b, "note": note}); contrasts.append(c)
    ...
    if {"oracle", "candidate_zero"} <= set(names):
        ...
        o = np.mean([smaps["oracle"][s] for s in seeds]); c = np.mean([smaps["candidate_zero"][s] for s in seeds])
        v = None
        if "verifier_argmax" in smaps:
            vseeds = sorted(set(smaps["verifier_argmax"]) & set(seeds))
            v = np.mean([smaps["verifier_argmax"][s] for s in vseeds]) if vseeds else None
        highlights.append({"type": "headroom", "oracle": float(o), "cz": float(c),
                           "verifier": float(v) if v is not None else None,
                           "captured": float((v - c) / (o - c)) if (v is not None and o > c) else None,
                           "k": 16, "note": f"paired on {len(seeds)} episodes; oracle is a diagnostic "
    ...
    for s in systems:
        for e in s["episodes"]:
            if e.get("traj"):
                goal_xy = load_json(base_path)["episodes"][0]["goal_xy"] if base_path.exists() else goal_xy
                break
        else:
            continue
        break
    ...
    if {"candidate_zero", "oracle"} <= set(names):
        ...
        for mode in ["near_miss", "misaligned", "mispositioned", "never_engaged"]:
            ids = [es for es, e in bmap.items() if es in omap and not e["success"] and e["failure_type"] == mode]
            rec = sum(omap[es]["success"] for es in ids)
            if ids:
                rows.append({"mode": mode, "count": len(ids), "recovered": rec})
        if rows:
            manifest["recovery"] = {"title": "Recovery by base failure mode (base fails → oracle succeeds)",
                                    "rows": rows}

    out_html = DASH / f"{args.name}.html"
    tpl = (DASH / "_viewer.html").read_text()
    out_html.write_text(tpl.replace("__ACTSEMBLE_DATA__", json.dumps(manifest, separators=(",", ":"))))
    (DASH / f"{args.name}.manifest.json").write_text(json.dumps(manifest, indent=1))
    print(f"[dashboard] {out_html}  ({len(systems)} systems, "
          f"{sum(len(s['episodes']) for s in systems)} episode records, {len(copied)} videos)")
    for s in systems:
        m = s["metrics"]
        print(f"    {s['label']:26s} {(m['success_rate'] or 0):.1%}  n={m['n']}  "
...
def cmd_record(args):
    ...
    cfg = json.loads(args.system)  # {"name","label","kind","color","selection":{...},"num_candidates":16,"components":[...]}
    policy = DiffusionPolicy.from_checkpoint(args.policy, device=args.device or "cuda", use_ema=True)
    comps = [ActionChunkCompatibility.from_checkpoint(p, device=args.device or "cuda") for p in cfg.get("components", [])]
    system = build_system({"policy": {"num_candidates": cfg.get("num_candidates", 16)},
                           "selection": cfg["selection"]}, policy, comps)
    ...
    def img():
        f = env.render(); f = f.detach().cpu().numpy() if torch.is_tensor(f) else np.asarray(f)
    ...
    for i, ep in enumerate(panel_episodes(panel)):
        ...
        obs_ad = ObservationAdapter(action_dim=policy.meta.action_dim)
        do_render = want_video and (ep.env_seed in vseeds if vseeds is not None else i < int(args.videos))
        s = np.asarray(obs_ad.observe(raw).state, np.float32).reshape(-1)
        obj = [s[24:26].copy()]; cov = [float(u.pseudo_render_intersection().item())]
        frames = [img()] if do_render else []
        succ_once = False; step = 0
        while step < 100 and not succ_once:
            ...
            s = np.asarray(obs_ad.observe(raw).state, np.float32).reshape(-1)
            obj.append(s[24:26].copy()); cov.append(float(u.pseudo_render_intersection().item()))
            if do_render:
                frames.append(img())
            succ_once = succ_once or (bool(info["success"].reshape(-1)[0]) if "success" in info else False)
            step += 1
            if bool(np.asarray(term).reshape(-1)[0]) or bool(np.asarray(trunc).reshape(-1)[0]):
                break
        goal = s[21:23]
        rows.append({"env_seed": int(ep.env_seed), "policy_sampling_seed": int(ep.policy_sampling_seed),
                     "success_once": bool(succ_once), "num_steps": int(step),
        ...
        if (i + 1) % 20 == 0:
            print(f"[record:{cfg['name']}] {i+1}/{panel.num_episodes} ({(time.time()-t0)/(i+1):.1f}s/ep)", flush=True)
    env.close()
    out = DASH / "captures" / f"{cfg['name']}.json"
    out.write_text(json.dumps({"system": {k: cfg[k] for k in ("name", "label", "kind", "color") if k in cfg},
                               "episodes": rows}))
    print(f"[record:{cfg['name']}] wrote {out} ({len(rows)} eps, "
...
def main():
    ...
    b = sub.add_parser("build"); b.add_argument("--name", default="selection_headroom"); b.add_argument("--title", default=None)
    r = sub.add_parser("record")
    r.add_argument("--system", required=True); r.add_argument("--policy", required=True)
    r.add_argument("--count", default=300); r.add_argument("--videos", default=0, type=int)
    r.add_argument("--video-seeds", default=None, help="comma-separated env_seeds to render video for")

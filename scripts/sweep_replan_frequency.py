...
def run_sweep(policy_ckpt, horizons, count, panel_name, out_dir: Path, device) -> float:
    ...
    for ha in horizons:
        ...
        print(
            f"[replan-hz] standalone H_a={ha} ({control_freq / ha:.2f} Hz) on {panel_name} "
            f"({count} ep) ...",
            flush=True,
        )
        evaluate_system(
            ...
            num_episodes=count,
            output_path=out_dir / f"eval_standalone_ha{int(ha):02d}.json",
            device=device,
            env=env,
            force=True,
        )
...
def build_and_report(out_dir: Path) -> dict | None:
    ...
    control_freq = (
        float(load_json(meta_p)["control_freq_hz"]) if meta_p.exists() else 20.0
    )
    ...
    for r in results:
        ...
        points.append(
            {
                "h_a": h_a(r),
                "replan_hz": control_freq / h_a(r),
                "success_rate": r["success_rate"],
                "wilson_ci": list(ci),
                "vs_slowest": {
                    "diff": r["success_rate"] - ref["success_rate"],
                    "mcnemar": mc,
                },
                "mean_replans": float(
                    np.mean([len(e["replans"]) for e in r["episodes"]])
                ),
            }
        )
    successes = [
        p["success_rate"] for p in sorted(points, key=lambda q: q["replan_hz"])
    ]
    monotone = all(b >= a - 1e-9 for a, b in zip(successes, successes[1:]))
    report = {
        "panel": results[0]["panel"]["name"],
        "num_episodes": n,
        "control_freq_hz": control_freq,
        "monotone_in_freq": monotone,
        "points": points,
    }
    ...
    print(
        f"[replan-hz] wrote {out_dir / 'replan_frequency.json'} and replan_frequency.png"
    )
    return report
...
def _format(rep: dict) -> str:
    ...
    for p in sorted(rep["points"], key=lambda q: q["h_a"], reverse=True):
        ...
        L.append(
            f"{p['h_a']:>4}{p['replan_hz']:>10.2f}{p['success_rate']:>10.1%}"
            f"{ci_s:>20}{diff_s:>15}"
        )
    L.append("-" * 74)
...
def _plot(rep: dict, path: Path) -> None:
    ...
    ax.set_title(
        f"More replanning → better — {rep['panel']} panel (n={rep['num_episodes']})"
    )

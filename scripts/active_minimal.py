...
def cmd_subset(args):
    if SUBSET.exists() and not args.force:
        print(f"[subset] {SUBSET} exists; skipping")
        return 0
    ...
    write_private_provenance(
        ...
        {
            "success_only": True,
            "conversion_method": "subset_of_active",
    ...
    print(
        f"[subset] wrote {SUBSET} : {s['num_episodes']} eps, {s['num_transitions']} transitions, "
...
def cmd_train_policy(args):
    ...
    if SEL_POLICY.exists() and not args.force:
        print(f"[train-policy] {SEL_POLICY} exists; skipping")
        return 0
    cfg = merge_config(
        load_config(REPO / SPEC["policy_config"]),
    ...
    best = max(
        hist, key=lambda r: (r["success_rate"], -r["step"])
    )  # max rate, tie->earlier
    shutil.copyfile(best["checkpoint_path"], SEL_POLICY)
    save_json(
        {
            "selected_step": best["step"],
            "screening_rate": best["success_rate"],
    ...
    print(
        f"[train-policy] best screening step {best['step']} = {best['success_rate']:.1%} "
        f"-> {SEL_POLICY.name}"
    )
    return 0
...
def cmd_sanity(args):
    ...
    print(
        f"[sanity] push-only n={N} policy dev success = {rate:.1%} "
        f"({dev['success_count']}/{dev['num_episodes']}) CI "
        f"[{dev['wilson_ci'][0]:.1%},{dev['wilson_ci'][1]:.1%}]"
    )
    if rate < SANITY_FLOOR:
        print(
            f"[sanity] FAIL: {rate:.1%} < floor {SANITY_FLOOR:.0%} — halting before compare"
        )
        return 2
...
def cmd_train_verifier(args):
    ...
    if SEL_VERIF.exists() and not args.force:
        print(f"[train-verifier] {SEL_VERIF} exists; skipping")
        return 0
    cfg = merge_config(
        load_config(REPO / SPEC["verifier_config"]),
    ...
    summary = train_component(
        component_cfg=cfg,
        dataset_path=SUBSET,
        output_dir=VERIF_DIR,
        device=args.device or "cuda",
    )
    sel = select_verifier(
        VERIF_DIR,
        primary=SPEC["verifier_selection"]["primary"],
        secondary=SPEC["verifier_selection"]["secondary"],
        force=args.force,
    )
    save_json(
        {
        ...
        VERIF_DIR / "select_summary.json",
    )
    print(
        f"[train-verifier] step {sel['selected_step']} ranking-acc "
...
def cmd_compare(args):
    ...
    eval_cfg = {
        ...
        "max_steps": 100,
        "perturbations": [],
        "video": {"max_success": 0, "max_failure": 0},
    }
    succ = {}
    for name, sel in SYSTEMS.items():
        ...
        if not dest.exists() or args.force:
            ...
            evaluate_system(
                system_cfg={"policy": {"num_candidates": 1}, "selection": sel},
                eval_cfg=eval_cfg,
                policy_checkpoint=str(SEL_POLICY),
                component_checkpoints=comps,
                output_path=dest,
                device=args.device or "cuda",
                env=None,
                num_candidates_override=16,
                force=args.force,
            )
        r = load_json(dest)
        succ[name] = np.array(r["successes"], dtype=int)
        print(
            f"[compare] {name:20s} {r['success_count']}/{r['num_episodes']} = {r['success_rate']:.1%}"
        )

    rows = {
        name: {"success_rate": float(s.mean()), "n": int(s.size)}
        for name, s in succ.items()
    }
    contrasts = {
        "verifier_vs_candidate_zero": _mcnemar(
            succ["verifier_argmax"], succ["candidate_zero"]
        ),
        "medoid_vs_candidate_zero": _mcnemar(
            succ["full_chunk_medoid"], succ["candidate_zero"]
        ),
        "verifier_vs_medoid": _mcnemar(
            succ["verifier_argmax"], succ["full_chunk_medoid"]
        ),
    }
    for k, c in contrasts.items():
        ...
        c["delta"] = (
            rows[
                {
                    "verifier": "verifier_argmax",
                    "medoid": "full_chunk_medoid",
                    "candidate_zero": "candidate_zero",
                }.get(a, a)
            ]["success_rate"]
            - rows[
                {"candidate_zero": "candidate_zero", "medoid": "full_chunk_medoid"}.get(
                    b, b
                )
            ]["success_rate"]
        )
    report = {
        "note": "MINIMALIST: 1 seed, 300-ep panel, screening-picked checkpoint. Rough image only.",
        "panel": COMPARE_PANEL,
        "num_candidates": 16,
        "systems": rows,
        "contrasts": contrasts,
    ...
    for k, c in contrasts.items():
        print(
            f"  {k:28s} Δ={c['delta']:+.1%}  McNemar z={c['z']:+.2f} "
...
def cmd_all(args):
    for fn in (cmd_subset, cmd_train_policy, cmd_sanity):
        ...
        if rc:  # sanity gate (rc=2) or error halts the chain
            print(f"[all] halted at {fn.__name__} (rc={rc})")
            return rc
...
def main():
    ...
    return {
        ...
        "sanity": cmd_sanity,
        "train-verifier": cmd_train_verifier,
        "compare": cmd_compare,
        "all": cmd_all,
    }[args.stage](args)

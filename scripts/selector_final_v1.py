...
SELECT_CFG = {
    ...
    "early_weighted_medoid": {
        "type": "early_weighted_medoid",
        "early_weight_decay": 0.25,
    },
...
def cmd_run(args):
    ...
    evaluate_system(
        ...
        eval_cfg=eval_cfg,
        policy_checkpoint=str(REPO / POLICY.format(k=seed)),
        component_checkpoints=_components(system, seed),
        output_path=dest,
    ...
    r = load_json(dest)
    print(
        f"[run] {system} seed{seed}: {r['success_count']}/{r['num_episodes']} = {r['success_rate']:.1%}"
    )
    return 0
...
def _paired(a_succ, b_succ):
    ...
    A = np.concatenate(a_succ)
    B = np.concatenate(b_succ)
    b01 = int(((A == 0) & (B == 1)).sum())  # b wins
    b10 = int(((A == 1) & (B == 0)).sum())  # a wins
    disc = b01 + b10
    z = (b10 - b01) / np.sqrt(disc) if disc else 0.0
    return {
        "per_seed_delta": [float(x) for x in deltas],
        "mean_delta": mean,
        "sd": sd,
        "ci95": ci,
        "sign_counts": {
            "a_gt_b": int((deltas > 0).sum()),
            "lt": int((deltas < 0).sum()),
            "eq": int((deltas == 0).sum()),
        },
        "pooled_episodes": int(len(A)),
        "a_wins": b10,
        "b_wins": b01,
        "mcnemar_z": float(z),
...
def _write_md(path, rep):
    L = [
        "# Selector baselines v1 — formal comparison",
        "",
    ...
    for s, v in rep["per_system"].items():
        if v["mean_success"] is not None:
            L.append(
                f"| {s} | {v['mean_success']:.1%} | {[round(x, 3) for x in v['per_seed_success']]} |"
            )
    L += [
        "",
        f"Best consensus by mean: **{rep['best_consensus_by_mean']}**",
        "",
    ...
    for name, c in rep["contrasts"].items():
        sc = c["sign_counts"]
        L.append(
            f"| {name.replace('__vs__', ' vs ')} | {c['mean_delta']:+.2%} | "
    ...
    L += [
        "",
        "Seed-level (5 seeds) is the conservative replication unit; the pooled "
...
def main():
    ...
    p.add_argument("stage", choices=["run", "analyze", "make-jobs"])
    ...
    if args.stage == "make-jobs":
        ...
        jobs = [
            {
                "script": "scripts/selector_final_v1.py",
        ...
        Path("outputs/selector_baselines_v1/final_v1/jobs.json").write_text(

...
SPEC_SOURCE = Path(
    os.environ.get(
        "ACTSEMBLE_PHASE0A_SPEC",
        REPO / "experiments" / "phase0a_v1" / "experiment_spec.yaml",
    )
)
SPEC_FROZEN = OUT / "experiment_spec.yaml"
SUBSET_DIR = Path(
    os.environ.get("ACTSEMBLE_PHASE0A_DATA", REPO / "data" / "phase0a_v1")
)
...
def cmd_init(args):
    ...
    if SPEC_FROZEN.exists():
        if load_config(SPEC_FROZEN) != load_config(SPEC_SOURCE):
            raise SystemExit(
                "Frozen spec differs from source — new experiment version required"
            )
    ...
    assert_panels_disjoint(
        panels_of(s),
        extra_seed_sets={
            "demonstrations": demonstration_seeds(REPO / s["dataset"]["master_path"])
        },
    )
...
def cmd_master_report(args):
    ...
    assert reader.dataset_hash == s["dataset"]["master_dataset_hash"], (
        "master hash changed!"
    )
    report = {
    ...
    print(
        f"[master-report] OK: {summary['num_episodes']} episodes, "
...
def cmd_make_subsets(args):
    ...
    for n in counts:
        ...
        if not out.exists():
            ...
            subprocess.run(
                [
                    ...
                    "--config",
                    str(REPO / s["dataset"]["data_config"]),
                    "--output",
                    str(out),
                    "--subset-size",
                    str(n),
        ...
        manifest = {
            ...
            "source_episode_ids": [
                e["source_episode_id"] for e in prov["exported_episodes"]
            ],
            "source_episode_seeds": [
                e["source_episode_seed"] for e in prov["exported_episodes"]
            ],
        ...
        print(
            f"[subsets] N={n}: {reader.num_transitions} transitions, "
...
def cmd_policy_pipeline(args):
    ...
    from actsemble.protocol.screening import (
        evaluate_checkpoint_on_panel,
    ...
    out_dir = (
        Path(args.out)
        if args.out
        else (OUT / "learning_curve" / f"ndemos_{n:04d}" / f"policy_seed_{seed}")
    )
    ...
    cfg = merge_config(
        load_config(REPO / s["policy_config"]), s.get("policy_config_overrides", {})
    )
    cfg = merge_config(
        cfg,
        {
    ...
    try:
        ...
        confirm = confirm_and_select(
            ...
            within_of_best=float(rule["within_of_best"]),
            force=args.force,
        )
        t_confirm = time.time() - t0
        t0 = time.time()
        dev = evaluate_checkpoint_on_panel(
            out_dir / "selected_policy.pt",
            panels["dataset_size_development"],
            env=env,
            device=device,
            max_steps=max_steps,
        )
    ...
    winner = next(
        c for c in confirm["candidates"] if c["step"] == confirm["selected_step"]
    )
    save_json(
        {
            "n_demos": n,
    ...
    print(
        f"[policy-pipeline] N={n} seed={seed}: selected step {confirm['selected_step']}, "
...
def cmd_verifier_pipeline(args):
    ...
    if (out_dir / "selected_verifier.pt").exists() and not args.force:
        ...
        return
    cfg = merge_config(
        load_config(REPO / s["verifier_config"]), s.get("verifier_config_overrides", {})
    )
    cfg = merge_config(
        cfg,
        {
    ...
    save_json(
        {
            ...
            "normalization_hash": summary["normalization_hash"],
            "selected_step": sel["selected_step"],
            "selected_verifier_hash": sel["selected_verifier_hash"],
            "winner_metrics": sel["winner_metrics"],
    ...
    print(
        f"[verifier-pipeline] seed={seed}: selected step {sel['selected_step']}, "
...
def cmd_learning_curve(args):
    ...
    for ndir in sorted(lc_dir.glob("ndemos_*")):
        ...
        half = (
            (t_critical_95(len(rates) - 1) * sd / np.sqrt(len(rates)))
            if len(rates) > 1
            else None
        )
        per_n[n] = {
            ...
            "seeds": [
                {
                    "policy_seed": x["policy_seed"],
                    "selected_step": x["selected_step"],
                    "screening_best": x["screening_best_rate"],
                    "confirmation": x["confirmation_rate"],
                    "dev_success": x["dev_success_rate"],
                    "dev_ci": x["dev_wilson_ci"],
                    "mean_episode_length": x["dev_mean_episode_length"],
                    "val_diffusion_loss_diagnostic": x[
                        "best_val_diffusion_loss_diagnostic_only"
                    ],
            ...
            "ci95_across_seeds": [mean - half, mean + half]
            if half is not None
            else None,
            "in_target_band": s["target_success_range"][0]
            <= mean
            <= s["target_success_range"][1],
        }
        for x in seeds:
            rows.append(
                (
                    ...
                    x["policy_seed"],
                    x["selected_step"],
                    x["screening_best_rate"],
                    x["confirmation_rate"],
                    x["dev_success_rate"],
                    x["best_val_diffusion_loss_diagnostic_only"],
                    x["train_wall_s"],
                )
            )
    ...
    with open(lc_dir / "results.csv", "w") as f:
        f.write(
            "n_demos,policy_seed,selected_step,screening_best,confirmation,"
            "dev_success,val_diffusion_loss,train_wall_s\n"
        )
        for r in sorted(rows):
            f.write(",".join(str(v) for v in r) + "\n")
    lines = [
        "# Phase 0A learning curve (dataset_size_development panel, 200 episodes)\n",
    ...
    for n in sorted(per_n):
        e = per_n[n]
        per_seed = ", ".join(
            f"s{x['policy_seed']}:{x['selected_step']}@{x['dev_success']:.0%}"
            for x in e["seeds"]
        )
        lines.append(
            f"| {n} | {e['mean_success']:.1%} | {e['std_success']:.1%} | "
...
def cmd_plot_learning_curve(args):
    ...
    ax.set_ylabel(
        "selected-policy success\n(dev panel, 200 episodes)", fontsize=10, color=ink2
    )
    ax.set_title(
        "PushT-v1 standalone Diffusion Policy learning curve — "
    ...
    ax.legend(
        handles=handles, loc="upper left", frameon=False, fontsize=8.5, labelcolor=ink2
    )
    fig.tight_layout()
...
def cmd_select_primary(args):
    ...
    record = {
        ...
        "selected_subset_hash": (
            load_json(OUT / "subset_manifests" / f"subset_{selected:04d}.json")[
                "subset_hash"
            ]
            if selected
            else None
        ...
        "reason": (
            ...
            + ", ".join(
                f"N={n}: {results['per_n'][str(n)]['mean_success']:.1%}"
                for n in sorted((int(k) for k in results["per_n"]), key=int)
            )
            if selected
            else "no candidate count in band — intermediate counts required"
        ),
    }
    ...
    for seed in [int(x) for x in s["final_policy_seeds"]]:
        ...
        if calib.exists() and (calib / "selected_policy.pt").exists():
            ...
            decisions.append(
                {
                    "policy_seed": seed,
                    "action": "requires_training",
                    "expected_dir": str(calib),
                }
            )
    save_json(
        {"n_demos_primary": n, "decisions": decisions},
...
def cmd_pair_setup(args):
    from actsemble.protocol.freeze import (
    ...
    if not (pair_dir / "freeze.json").exists() or args.force:
        manifest = build_freeze_manifest(
            ...
            verifier_path=OUT
            / "verifier_runs"
            / f"verifier_seed_{seed}"
            / "selected_verifier.pt",
            dataset_path=subset_path(n),
            panels=panels_of(s),
    ...
    try:
        report = run_integration(
            seed_dir=pair_dir,
            freeze=load_freeze(pair_dir),
            panel=panels_of(s)["integration"],
            env=env,
            device=args.device or "cuda",
            max_steps=int(s["episode_max_steps"]),
            force=args.force,
...
def cmd_pair_final(args):
    ...
    mirror = {
        ...
        "control": "candidate_zero_control",
        "actsemble": "actsemble",
    }
...
def cmd_manifest(args):
    ...
    seed_banks = {
        name: {
            ...
            "policy_sampling_seeds": [
                e.policy_sampling_seed for e in panel_episodes(p)
            ],
        }
    ...
    print(
        f"[manifest] written; git {manifest['git_commit'][:12]}, "
...
def _pair_results(seed: int, where: str) -> dict:
    ...
    return {
        name: load_json(base / f"eval_{name}.json")
    ...
    for seed in seeds:
        for where, tag in (
            ...
            same_replans = sum(
                sum(
                    x == y
                    for x, y in zip(ea["candidate_hashes"], eb["candidate_hashes"])
                )
                for ea, eb in zip(sa["episodes"], co["episodes"])
            )
...
def cmd_aggregate(args):
        ...
        per_pair.append(
            {
                ...
                "paired_win": counts["a_wins"],
                "paired_loss": counts["b_wins"],
    ...
    def across(key):
        ...
        half = (
            t_critical_95(len(vals) - 1) * sd / np.sqrt(len(vals))
            if len(vals) > 1
            else None
        )
        return {
            "per_pair": vals.tolist(),
            "mean": m,
            "std": sd,
    ...
    result = {
        ...
        "episodes_per_pair": per_pair
        and _pair_results(seeds[0], "final_test/nominal")["standalone"]["num_episodes"],
        "per_pair": per_pair,
        "across_pairs": {
        ...
        "note": (
            "Seed pairs are the replication unit; the pooled episodes are NOT "
    ...
    with open(ana / "paired_results.csv", "w") as f:
        f.write(
            "pair,standalone,control,actsemble,delta_vs_standalone,delta_vs_control,"
    ...
    print(
        f"[aggregate] Actsemble - standalone across {len(per_pair)} pairs: "

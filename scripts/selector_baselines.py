...
VERIFIER = (
    REPO / "outputs/phase0a_v1/verifier_runs/verifier_seed_4/selected_verifier.pt"
)
...
SYSTEMS = {
    ...
    "full_chunk_medoid": (
        {"policy": {"num_candidates": 1}, "selection": {"type": "full_chunk_medoid"}},
        False,
    ),
    "early_weighted_medoid": (
        {
            "policy": {"num_candidates": 1},
            "selection": {"type": "early_weighted_medoid", "early_weight_decay": 0.25},
        },
        False,
    ),
    "coordinate_median_projection": (
        {
            "policy": {"num_candidates": 1},
            "selection": {"type": "coordinate_median_projection"},
        },
        False,
    ),
    "largest_cluster_medoid": (
        {
            "policy": {"num_candidates": 1},
            "selection": {"type": "largest_cluster_medoid"},
        },
        False,
    ),
    "verifier_argmax": (
...
def cmd_integration(args):
    ...
    def check(name, ok, detail=""):
        ...
        print(
            f"[selector-int] {'PASS' if ok else 'FAIL'} | {name}"
    ...
    gen = policy.new_generator(
        derive_seed(777, "candidates", policy.checkpoint_hash, 0)
    )
    shared = policy.sample_action_chunks(history, num_samples=K, generator=gen)
    shared_hash = hashlib.sha256(
        np.ascontiguousarray(shared.detach().cpu().numpy().astype(np.float32)).tobytes()
    ).hexdigest()[:16]
    ...
    for name, (cfg, needs_v) in SYSTEMS.items():
        ...
        rec = {}
        seen = shared.clone()
        seen_hash = hashlib.sha256(
            np.ascontiguousarray(
                seen.detach().cpu().numpy().astype(np.float32)
            ).tobytes()
        ).hexdigest()[:16]
    ...
    check(
        "identical_candidate_tensor_before_selection",
        hashes == {shared_hash},
        f"{len(hashes)} distinct tensor hash(es)",
    )
    # 4. differences only in selected index
    check(
        "differences_only_in_selected_index",
        len({v["selected_index"] for v in per_system.values()}) >= 1
        and all(v["seen_tensor_hash"] == shared_hash for v in per_system.values()),
        "selected: "
        + ", ".join(f"{k}={v['selected_index']}" for k, v in per_system.items()),
    )
    ...
    check(
        "no_component_for_consensus",
        all(
            per_system[n]["n_components"] == 0
            for n in SYSTEMS
            if n != "verifier_argmax"
        )
        and per_system["verifier_argmax"]["n_components"] == 1,
    )
    # 7. reset clears diagnostics
    sys_r = build_system(
        SYSTEMS["full_chunk_medoid"][0], policy, [], candidate_root_seed=777
    )
    sys_r._replan_records.append({"dummy": 1})
    sys_r.reset(episode_seed=1)
    check(
        "reset_clears_diagnostics",
    ...
    for name, (cfg, needs_v) in SYSTEMS.items():
        ...
        system = build_system(
            {**cfg, "policy": {**cfg["policy"], "num_candidates": K}},
            policy,
            comps,
        ...
        latency[name] = {
            "mean_policy_latency_s": float(
                np.mean([r["policy_latency_s"] for r in rp])
            ),
            "mean_selector_latency_s": float(
                np.mean(
                    [
                        r.get("selector_latency_s", r.get("component_latency_s", 0.0))
                        for r in rp
                    ]
                )
            ),
    ...
    check(
        "selector_latency_measured_separately",
        all(latency[n]["mean_selector_latency_s"] >= 0 for n in latency),
        "; ".join(
            f"{n}: sel={latency[n]['mean_selector_latency_s'] * 1e3:.2f}ms "
            f"pol={latency[n]['mean_policy_latency_s'] * 1e3:.1f}ms"
            for n in latency
        ),
    )
    env.close()
...
def cmd_dev_check(args):
    ...
    for name, (cfg, needs_v) in SYSTEMS.items():
        ...
        results[name] = evaluate_system(
            ...
            component_checkpoints=comps,
            output_path=out / f"eval_{name}.json",
        ...
        r = results[name]
        print(
            f"[selector-dev] {name}: {r['success_count']}/{r['num_episodes']} "
            f"= {r['success_rate']:.1%} (NON-CLAIM)"
        )

    # candidate identity up to first selection divergence (bitwise hashes)
    ...
    def sel_stats(r):
        ...
        pw = [
            rp.get("mean_pairwise_distance")
            for e in r["episodes"]
            for rp in e["replans"]
            if rp.get("mean_pairwise_distance") is not None
        ]
        sel_lat = [
            rp.get("selector_latency_s")
            for e in r["episodes"]
            for rp in e["replans"]
        ...
        return {
            ...
            "selector_change_rate_vs_candidate_zero": float(np.mean(changed))
            if changed
            else 0.0,
            "selected_index_distribution": dict(Counter(idxs)),
            "mean_pairwise_candidate_distance": float(np.mean(pw)) if pw else None,
            "mean_selector_latency_ms": float(np.mean(sel_lat) * 1e3)
            if sel_lat
            else None,
    ...
    for name, r in results.items():
        ...
        eps = [
            e["episode_index"]
            for e, e0 in zip(r["episodes"], cz["episodes"])
    ...
    report = {
        ...
        "panel": DEV_PANEL.to_dict(),
        "candidate_identity_until_divergence": identity or "OK (bitwise prefix)",
        "per_selector": stats,
...
def _write_dev_markdown(path, report, stats):
    lines = [
        "# Selector development-panel check (NON-CLAIM diagnostics)",
        "",
    ...
    for name, s in stats.items():
        lines.append(
            f"| {name} | {s['success_rate_nonclaim']:.1%} | "
...
def main():
    ...
    args = p.parse_args()
    return {"integration": cmd_integration, "dev-check": cmd_dev_check}[args.stage](
        args
    )

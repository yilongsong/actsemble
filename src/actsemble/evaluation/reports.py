...
def verify_candidate_identity(results: list[dict]) -> list[str]:
    ...
    Candidate hashes must match exactly. Numerical summaries are diagnostics,
    not evidence of identity. Systems run with a different K are skipped."""
    problems = []
    unverifiable = [
        r.get("system_name")
        for r in results
        if r.get("episodes")
        and not all(e.get("candidate_hashes") for e in r["episodes"])
    ]
    if unverifiable:
        problems.append(f"candidate identity data missing for system(s) {unverifiable}")
    verifiable = [
        r
        for r in results
    ...
    for i, a in enumerate(verifiable):
        for b in verifiable[i + 1 :]:
            ...
            if len(a["episodes"]) != len(b["episodes"]):
                problems.append(
                    f"candidate identity cannot be checked: {a['system_name']} and "
                    f"{b['system_name']} have different episode counts"
                )
                continue
            bad_episodes = []
            for ea, eb in zip(a["episodes"], b["episodes"]):
                ...
                if len(sa) != len(ha) or len(sb) != len(hb):
                    bad_episodes.append(ea["episode_index"])
                    continue
                shared = min(len(ha), len(hb))
                divergence = next((k for k in range(shared) if sa[k] != sb[k]), None)
                must_match = shared if divergence is None else divergence + 1
                for k in range(must_match):
                    if ha[k] != hb[k]:
                        bad_episodes.append(ea["episode_index"])
                        break
                else:
                    if divergence is None and len(ha) != len(hb):
                        bad_episodes.append(ea["episode_index"])
            if bad_episodes:
                problems.append(
                    f"candidate-set identity violated: {a['system_name']} vs "
                    f"{b['system_name']} differ bitwise before any selection "
...
def format_report(report: dict) -> str:
    ...
    for name, p in report["pairwise_vs_baseline"].items():
        ...
        lines.append(
            f"  decision-latency overhead: {p['latency_overhead_s'] * 1000:+.1f} ms/step"
        )
        change = report["systems"][name].get("selection_change_rate")
        if change is not None:
            lines.append(
                f"  candidate-selection-change frequency: {change:.1%} of replans"
            )
        lines.append("")
    return "\n".join(lines)

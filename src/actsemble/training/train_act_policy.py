...
(enforced by tests/training/test_training_has_no_sim_dependency.py), same seed discipline
...
def train_act_policy(
    *,
    ...
    if episode_sampling:
        train_ds: Dataset = ACTEpisodeDataset(
            train_eps,
            normalizer,
            ...
            start_seed=gen_seeds["act_sampling"],
        )
    else:
    ...
    if not val_eps:
        ...
        episode_sampling
    ):  # episode-weighted + deterministic (one fixed window per episode)
        val_ds = ACTEpisodeDataset(
            ...
            prediction_horizon=meta.prediction_horizon,
            include_previous_action=meta.include_previous_action,
            start_seed=gen_seeds["act_sampling"] + 1,
            fixed=True,
        )
    else:
        val_ds = DiffusionWindowDataset(
            val_eps,
            normalizer,
            obs_horizon=meta.obs_horizon,
            prediction_horizon=meta.prediction_horizon,
    ...
    train_size = len(cast(Any, train_ds))
    loader = DataLoader(
        train_ds,
    ...
    val_loader = (
        ...
        if val_ds is not None and val_ds.__len__() > 0
        else None
    )
    ...
    use_ema = bool(
        tcfg.get("use_ema", True)
    )  # canonical ACT: False (eval best-val raw weights)
    ...
    mask_padding = bool(
        tcfg.get("mask_padded_actions", True)
    )  # canonical ACT masks padding
    grad_clip = float(tcfg.get("gradient_clip_norm", 1.0))
    total_steps = resolve_total_steps(tcfg, train_size, batch_size, max_steps)
    steps_per_epoch = max(
        1, train_size // batch_size
    )  # episode-weighted when episode_sampling
    ...
    def act_loss(m, batch, lgen):
        ...
        amask = (
            batch["action_mask"].to(device) if mask_padding else None
        )  # [B, H_p] real=True
        ...
        if amask is not None:
            ...
            # Canonical ACT applies the padding mask and then averages over the
            # full fixed-size action tensor. This keeps the published KL/L1
            # weighting convention unchanged near episode boundaries.
            recon = (F.l1_loss(pred, x0, reduction="none") * w).mean()
        else:
    ...
    def validate():
        ...
        with torch.no_grad():
            for vb in val_loader:
                ...
                weighted_loss += act_loss(m, vb, vgen)[0].item() * batch_n
                examples += batch_n

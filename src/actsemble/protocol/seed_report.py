...
from ..evaluation.reports import verify_comparable
from .freeze import load_freeze, verify_result_against_freeze

# Two-sided 95% Student-t critical values for the small-df regime used here.
_T_95 = {
    1: 12.706,
    2: 4.303,
    3: 3.182,
...
def t_critical_95(df: int) -> float:
    ...
    if df in _T_95:
        return _T_95[df]
    # Cornish-Fisher expansion around the normal 97.5% quantile.
    z = 1.959963984540054
    v = float(df)
...
def aggregate_experiment(
    experiment_dir: str | Path, *, regime: str = "nominal"
) -> dict:
    ...
    for sdir in sorted(experiment_dir.glob("seed_*")):
        ...
        if not final_dir.exists():
            continue
        rows = {
            name: load_json(final_dir / f"eval_{name}.json")
        ...
        freeze_problems = [
            ...
            for problem in verify_result_against_freeze(row, freeze)
        ]
        comparison_problems = verify_comparable(list(rows.values()))
        if freeze_problems or comparison_problems:
            raise ValueError(
                f"Cannot aggregate incomparable results in {sdir}:\n  - "
                + "\n  - ".join(freeze_problems + comparison_problems)
            )
        per_seed.append(
            {
                "seed_dir": sdir.name,
                ...
                "actsemble_minus_standalone": rows["actsemble"]["success_rate"]
                - rows["standalone"]["success_rate"],
                "actsemble_minus_control": rows["actsemble"]["success_rate"]
                - rows["control"]["success_rate"],
            }
        )
    if not per_seed:
    ...
    report = {
        ...
        "per_seed": per_seed,
        "note": (
            "Training seeds are the replication unit; episode-level paired "
...
def format_aggregate(report: dict) -> str:
    ...
    for key, label in (
        ("actsemble_minus_standalone", "Actsemble − standalone"),
        ...
        lines.append(
            f"  mean {st['mean_difference']:+.2%}"
            + (
                f", sd {st['std_difference']:.2%}"
                if st["std_difference"] is not None
                else ""
            )
        )
        if ci:
            lines.append(
                f"  95% CI across seeds [{ci[0]:+.2%}, {ci[1]:+.2%}] ({st['ci_method']})"
            )
        lines.append(
            f"  seeds +/−/0: {st['positive_seeds']}/{st['negative_seeds']}/{st['zero_seeds']}"
        )
    return "\n".join(lines)

...
SYSTEM_CFGS = {
    ...
    "control": {
        "policy": {"num_candidates": 1},
        "selection": {"type": "first_candidate"},
    },
    "actsemble": {
        "policy": {"num_candidates": 1},
...
def run_integration(
    *,
    ...
    def check(name: str, passed: bool, detail: str = "") -> None:
        ...
        print(
            f"[integration] {'PASS' if passed else 'FAIL'} | {name}"
    ...
    check(
        "dataset_subset_hash_compatibility",
        all(
            not verify_result_against_freeze(r, freeze)
    ...
    check(
        "history_reset_between_episodes",
        ran_replans > 0
        and probe.diagnostics()["num_replans"] == 0
        and len(probe._history) == 0
        and len(probe._queue) == 0,
    )
    ...
    standalone_control_full = all(
        ...
        for ea, eb in zip(
            results["standalone"]["episodes"], results["control"]["episodes"]
        )
    )
    check(
        "identical_candidate_tensors",
        not identity_problems and standalone_control_full,
        "; ".join(identity_problems)
        or "prefix rule + full standalone/control equality",
    )

    # 7. finite verifier scores on a probe episode
    probe.reset(episode_seed=0)
    result, _ = run_panel_episode(
        env, probe, ep0, max_steps=min(max_steps, 20), pert_specs=[]
    )
    scores = [
        s for r in probe.diagnostics()["replans"] for s in r.get("component_scores", [])
    ]
    check(
        "finite_verifier_scores",
        bool(
            len(scores) > 0
    ...
    faulty = CandidateRerankingActsemble(
        policy, _ExplodingVerifier(verifier), num_candidates=k
    )
    fresult, _ = run_panel_episode(
        env, faulty, ep0, max_steps=min(max_steps, 20), pert_specs=[]
    )
    fdiag = faulty.diagnostics()
    check(
        "fallback_behavior",
        fresult.exception is None
        and fdiag["fallback_count"] == fdiag["num_replans"] > 0
    ...
    report = {
        ...
        "panel": panel.to_dict(),
        "note": (
            "Integration success rates are for implementation checking only — "
